...
mydata %>% 
  ...
  ggplot(aes(x=current_status, y= ..prop.., group=1))+
  geom_bar(fill="steelblue", width=0.5)+
...
mydata %>% 
  ...
  ggplot(aes(x=current_status, y= ..prop.., group=1))+
  geom_bar(fill="steelblue", width=0.5)+
  ggtitle("(In the past 12 months/Since you started your current job/In the last year of your most recent job), how often on your primary job (have you spent/have you spent/did you spend) less time with your family than you wanted to because of work responsibilities?") +
  xlab("subject's response")
```
...
number_of_jobs <- mydata %>% 
  ...
  summarize(n = n(),
            min = min(H4LM12 ),
            q1 = quantile(H4LM12 , probs=0.25),
            median = median(H4LM12 ),
            mean = mean(H4LM12 ),
            q3 = quantile(H4LM12 , probs=0.75),
            max = max(H4LM12 ),
            sd = sd(H4LM12 ))
